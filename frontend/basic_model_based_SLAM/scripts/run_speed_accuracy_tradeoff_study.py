...
def plot_tradeoff(rows, output_png):
    plt.figure(figsize=(6.6, 5.2))
    for row in rows:
        plt.scatter(row["mean_runtime_ms"], row["ate_rmse_m"], s=85)
        plt.text(row["mean_runtime_ms"] + 1.0, row["ate_rmse_m"], row["label"], fontsize=9)
    plt.xlabel("Mean preprocessing runtime per frame (ms)")
...
def main():
    parser = argparse.ArgumentParser(description="Run speed-accuracy tradeoff study for foundation-enhanced RGB-D SLAM.")
        ...
        [
            {
                "label": "fast",
                "mode": "filtered",
                "config": os.path.join(args.project_root, "config", "world_sam_pipeline_foundation_panoptic_person_v2_fast_local.json"),
                "output_dir": os.path.join(args.output_root, "fast"),
            },
            {
                "label": "balanced",
                "mode": "filtered",
                "config": os.path.join(args.project_root, "config", "world_sam_pipeline_foundation_panoptic_person_v2_local.json"),
                "output_dir": os.path.join(args.output_root, "balanced"),
            },
            {
                "label": "accurate",
                "mode": "filtered",
                "config": os.path.join(args.project_root, "config", "world_sam_pipeline_foundation_panoptic_person_v2_accurate_local.json"),
                "output_dir": os.path.join(args.output_root, "accurate"),
            },
        ]
    )
    ...
    csv_path = os.path.join(args.output_root, "speed_accuracy_tradeoff_summary.csv")
    ...
    json_path = os.path.join(args.output_root, "speed_accuracy_tradeoff_summary.json")
    with open(json_path, "w", encoding="utf-8") as f:
        json.dump(rows, f, indent=2, ensure_ascii=False)

    plot_tradeoff(rows, os.path.join(args.output_root, "speed_accuracy_tradeoff.png"))
