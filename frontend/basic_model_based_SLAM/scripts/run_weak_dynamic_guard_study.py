...
def main():
    parser = argparse.ArgumentParser(description="Run weak-dynamic guard benchmark on one RGB-D sequence.")
    ...
    studies = [
        ...
        {
            "label": "ours_dynamic_memory",
            "mode": "filtered",
            "config": os.path.join(args.project_root, "config", "world_sam_pipeline_foundation_panoptic_person_v2_local.json"),
            "output_dir": os.path.join(args.output_root, "ours_dynamic_memory"),
        },
        {
            "label": "ours_weak_dynamic_guard",
            "mode": "filtered",
            "config": os.path.join(args.project_root, "config", "world_sam_pipeline_foundation_panoptic_person_v2_weak_dynamic_guard_local.json"),
            "output_dir": os.path.join(args.output_root, "ours_weak_dynamic_guard"),
        },
        {
            "label": "ours_motion_guarded_memory",
            "mode": "filtered",
            "config": os.path.join(args.project_root, "config", "world_sam_pipeline_foundation_panoptic_person_v3_motion_guard_local.json"),
            "output_dir": os.path.join(args.output_root, "ours_motion_guarded_memory"),
        },
    ]
    ...
    csv_path = os.path.join(args.output_root, "weak_dynamic_guard_summary.csv")
    ...
    json_path = os.path.join(args.output_root, "weak_dynamic_guard_summary.json")
