...
public:
    ...
    int GetFeatureInstanceId(size_t idx) const {
        return (idx < mvInstanceIds.size()) ? mvInstanceIds[idx] : -1;
    }

    int GetFeatureSemanticLabel(size_t idx) const {
    ...
    bool GetPredictedInstanceMotion(int instanceId, Sophus::SE3f& motion) const {
        ...
        if(it == mmPredictedInstanceMotions.end())
            return false;
        motion = it->second;
        return true;
    }

    const std::vector<DynamicInstancePointObservation>& GetDynamicInstancePointObservations() const {
        return mvDynamicInstancePointObservations;
    }

    void SetImageFrameWindow(const std::vector<WindowFrameSnapshot>& vFrames);
...
protected:
    ...
    std::vector<WindowFrameSnapshot> mvImageFrameWindow;
