...
struct DynamicInstancePointObservation
{
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW

    DynamicInstancePointObservation()
        : instanceId(-1),
          semanticLabel(0),
          featureIdx(-1),
          qualityWeight(1.0),
...
public:
    ...
    void AssignPanopticObservation(const PanopticFrameObservation& panopticObservation);

    inline bool HasPanopticObservation() const {
        return mbHasPanopticMask;
    }

    inline int GetFeaturePanopticId(size_t idx) const {
        return (idx < mvPanopticIds.size()) ? mvPanopticIds[idx] : -1;
    }

    inline int GetFeatureSemanticLabel(size_t idx) const {
        return (idx < mvSemanticLabels.size()) ? mvSemanticLabels[idx] : -1;
    }

    inline int GetFeatureInstanceId(size_t idx) const {
        return (idx < mvInstanceIds.size()) ? mvInstanceIds[idx] : -1;
    }

    inline int GetNumThingFeatures() const {
        return mnPanopticThingFeatures;
    }

    inline int GetNumStuffFeatures() const {
        return mnPanopticStuffFeatures;
    }
...
public:
    ...
    // STSLAM panoptic labels aligned with left/monocular ORB features.
    PanopticFrameObservation mPanopticObservation;
    vector<int> mvPanopticIds;
    vector<int> mvSemanticLabels;
    vector<int> mvInstanceIds;
    // Sparse image-geometry cue aligned with left/monocular ORB features:
    // -1 = dynamic evidence, 0 = unknown, 1 = static evidence.
    vector<int> mvSparseFlowGeometryLabels;
    vector<float> mvSparseFlowForwardBackwardErrors;
    vector<float> mvSparseFlowEpipolarErrors;
    map<int, InstanceObservation> mmInstanceObservations;
    map<int, Sophus::SE3f> mmPredictedInstanceMotions;
    vector<DynamicInstancePointObservation> mvDynamicInstancePointObservations;
...
struct WindowFrameSnapshot
{
    ...
    WindowFrameSnapshot();
    explicit WindowFrameSnapshot(const Frame& frame);
    ...
    inline int GetLeftFeatureCount() const {
        return NLeft;
    }
