...
Instance* Atlas::AddInstance(Instance* pInstance)
{
    unique_lock<mutex> lock(mMutexAtlas);
    if(!mpCurrentMap)
        return static_cast<Instance*>(NULL);

    return mpCurrentMap->AddInstance(pInstance);
}
...
std::vector<Instance*> Atlas::GetAllInstances()
{
    unique_lock<mutex> lock(mMutexAtlas);
    return mpCurrentMap->GetAllInstances();
}
...
Instance* Atlas::GetInstance(const int instanceId)
{
    unique_lock<mutex> lock(mMutexAtlas);
    if(!mpCurrentMap)
        return static_cast<Instance*>(NULL);
    return mpCurrentMap->GetInstance(instanceId);
}
