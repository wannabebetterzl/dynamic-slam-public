    ...
    mvKeysRight(F.mvKeysRight), NLeft(F.Nleft), NRight(F.Nright), mTrl(F.GetRelativePoseTrl()), mnNumberOfOpt(0), mbHasVelocity(false)
{
    ...
    for(const auto& entry : F.mmInstanceObservations)
        mspInstanceIds.insert(entry.first);

    if(mspInstanceIds.empty())
    {
        for(const int instanceId : mvInstanceIds)
        {
            if(instanceId >= 0)
                mspInstanceIds.insert(instanceId);
        }
    }
...
void KeyFrame::SetImageFrameWindow(const std::vector<WindowFrameSnapshot>& vFrames)
{
    unique_lock<mutex> lock(mMutexFeatures);
    mvImageFrameWindow = vFrames;
}

std::vector<WindowFrameSnapshot> KeyFrame::GetImageFrameWindow() const
{
    unique_lock<mutex> lock(const_cast<std::mutex&>(mMutexFeatures));
    return mvImageFrameWindow;
}

std::vector<WindowFrameSnapshot>& KeyFrame::GetMutableImageFrameWindow()
{
    return mvImageFrameWindow;
}
