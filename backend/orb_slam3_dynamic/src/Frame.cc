...
WindowFrameSnapshot::WindowFrameSnapshot(const Frame& frame)
    : mnFrameId(frame.mnId),
      mTimeStamp(frame.mTimeStamp),
      mTcw(),
      mbHasPose(frame.HasPose()),
      mpCamera(frame.mpCamera),
      mpCamera2(frame.mpCamera2),
      mTrl(frame.GetRelativePoseTrl()),
      fx(frame.fx),
      fy(frame.fy),
      cx(frame.cx),
      cy(frame.cy),
      ...
      mvInstanceIds(frame.mvInstanceIds),
      mvSemanticLabels(frame.mvSemanticLabels),
...
Frame::Frame(const Frame &frame)
    :mpcpi(frame.mpcpi),mpORBvocabulary(frame.mpORBvocabulary), mpORBextractorLeft(frame.mpORBextractorLeft), mpORBextractorRight(frame.mpORBextractorRight),
     ...
     mPanopticObservation(frame.mPanopticObservation),
     mvPanopticIds(frame.mvPanopticIds), mvSemanticLabels(frame.mvSemanticLabels),
...
void Frame::AssignPanopticObservation(const PanopticFrameObservation& panopticObservation)
{
    mPanopticObservation = PanopticFrameObservation();
    mvPanopticIds.clear();
    mvSemanticLabels.clear();
    mvInstanceIds.clear();
    mmInstanceObservations.clear();
    mmPredictedInstanceMotions.clear();
    mbHasPanopticMask = false;
    mnPanopticThingFeatures = 0;
    mnPanopticStuffFeatures = 0;

    if(N <= 0 || panopticObservation.empty())
        return;

    const cv::Mat& panopticMask = panopticObservation.rawPanopticMask;
    ...
    mPanopticObservation = panopticObservation;
    mvPanopticIds.assign(N, -1);
    mvSemanticLabels.assign(N, -1);
    mvInstanceIds.assign(N, -1);
    mbHasPanopticMask = true;
    for(const auto& entry : panopticObservation.instances)
        mmInstanceObservations[entry.trackId] = entry;

    const int numUsableKeys = std::min(N, static_cast<int>(mvKeys.size()));
    for(int i = 0; i < numUsableKeys; ++i)
    {
        const int x = static_cast<int>(mvKeys[i].pt.x);
        const int y = static_cast<int>(mvKeys[i].pt.y);
        if(x < 0 || x >= panopticMask.cols || y < 0 || y >= panopticMask.rows)
            continue;
        ...
        mvPanopticIds[i] = panopticId;

        if(panopticId <= 0)
        {
            mvSemanticLabels[i] = 0;
            mvInstanceIds[i] = 0;
        ...
        int trackId = instanceId;

        const auto observationIt = mmInstanceObservations.find(instanceId);
        if(observationIt != mmInstanceObservations.end())
            trackId = observationIt->second.trackId;

        mvSemanticLabels[i] = semanticId;
        mvInstanceIds[i] = (trackId > 0) ? trackId : 0;

        if(trackId > 0)
            ++mnPanopticThingFeatures;
        else
            ++mnPanopticStuffFeatures;
    }
}
...
std::vector<int> Frame::GetFeatureIndicesForInstance(const int instanceId) const
{
    ...
    indices.reserve(mvInstanceIds.size());
    for(size_t i = 0; i < mvInstanceIds.size(); ++i)
    {
        if(mvInstanceIds[i] == instanceId)
