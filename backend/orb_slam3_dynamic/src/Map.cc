...
Instance* Map::AddInstance(Instance* pInstance)
{
    if(!pInstance)
    ...
    const auto it = mmpInstances.find(instanceId);
    if(it != mmpInstances.end())
        return it->second;

    mmpInstances[instanceId] = pInstance;
    return pInstance;
}
...
void Map::EraseInstance(int instanceId)
{
    unique_lock<mutex> lock(mMutexMap);
    mmpInstances.erase(instanceId);
}
...
vector<Instance*> Map::GetAllInstances()
{
    unique_lock<mutex> lock(mMutexMap);
    vector<Instance*> vInstances;
    vInstances.reserve(mmpInstances.size());
    for(const auto& entry : mmpInstances)
        vInstances.push_back(entry.second);
    return vInstances;
}

Instance* Map::GetInstance(int instanceId)
{
    unique_lock<mutex> lock(mMutexMap);
    const auto it = mmpInstances.find(instanceId);
    if(it == mmpInstances.end())
...
void Map::RecordInstanceClassSizePrior(int semanticLabel, double radius)
{
    RecordInstanceClassSizePrior(semanticLabel, 0, radius);
}

void Map::RecordInstanceClassSizePrior(int semanticLabel, int instanceId, double radius)
{
    if(semanticLabel <= 0 || radius <= 0.0)
        return;

    unique_lock<mutex> lock(mMutexMap);
    InstanceClassSizePrior& prior = mmInstanceClassSizePriors[semanticLabel];
    prior.sumRadius += radius;
    prior.sumRadiusSq += radius * radius;
    ++prior.count;
    if(instanceId > 0)
    {
        InstanceClassSizePrior::RadiusAccumulator& instancePrior =
            prior.perInstance[instanceId];
        instancePrior.sumRadius += radius;
        instancePrior.sumRadiusSq += radius * radius;
        ++instancePrior.count;
    }
}

std::map<int, Map::InstanceClassSizePrior> Map::GetAllInstanceClassSizePriors()
{
    unique_lock<mutex> lock(mMutexMap);
    return mmInstanceClassSizePriors;
}
