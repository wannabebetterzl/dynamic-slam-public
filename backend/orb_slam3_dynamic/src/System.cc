...
{
    return TrackRGBD(im, depthmap, timestamp, cv::Mat(), vImuMeas, filename);
}

Sophus::SE3f System::TrackRGBD(const cv::Mat &im, const cv::Mat &depthmap, const double &timestamp, const cv::Mat &panopticMask, const vector<IMU::Point>& vImuMeas, string filename)
{
    return TrackRGBD(im, depthmap, timestamp, panopticMask, cv::Mat(), vImuMeas, filename);
}

Sophus::SE3f System::TrackRGBD(const cv::Mat &im, const cv::Mat &depthmap, const double &timestamp, const cv::Mat &panopticMask, const cv::Mat &dynamicDepthmap, const vector<IMU::Point>& vImuMeas, string filename)
{
    ...
    cv::Mat dynamicDepthToFeed = dynamicDepthmap.empty() ? cv::Mat() : dynamicDepthmap.clone();
    if(settings_ && settings_->needToResize()){
        ...
        if(!dynamicDepthmap.empty())
            cv::resize(dynamicDepthmap,dynamicDepthToFeed,settings_->newImSize());
    }
    ...
    Sophus::SE3f Tcw = mpTracker->GrabImageRGBD(imToFeed,imDepthToFeed,timestamp,panopticMask,dynamicDepthToFeed,filename);
...
{
    return TrackMonocular(im, timestamp, cv::Mat(), vImuMeas, filename);
}

Sophus::SE3f System::TrackMonocular(const cv::Mat &im, const double &timestamp, const cv::Mat &panopticMask, const vector<IMU::Point>& vImuMeas, string filename)
{
    ...
    Sophus::SE3f Tcw = mpTracker->GrabImageMonocular(imToFeed,timestamp,panopticMask,filename);
...
void System::SaveTrajectoryTUM(const string &filename)
{
    ...
    if(vpKFs.empty())
    {
        cerr << "No keyframes available. Skipping camera trajectory export." << endl;
        return;
    }
...
void System::SaveKeyFrameTrajectoryTUM(const string &filename)
{
    ...
    if(vpKFs.empty())
    {
        cerr << "No keyframes available. Skipping keyframe trajectory export." << endl;
        return;
    }
...
bool System::LocalMappingAcceptKeyFrames()
{
    return mpLocalMapper && mpLocalMapper->AcceptKeyFrames();
}
