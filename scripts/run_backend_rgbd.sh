...
case "$PROFILE" in
  ...
  geom_dynamic_reject)
    ...
    export STSLAM_GEOMETRIC_DYNAMIC_REJECTION_MIN_STATIC_MAP_OBSERVATIONS=1
  ...
  candidate_sparseflow_tracklocal)
    export STSLAM_SEMANTIC_GEOMETRIC_VERIFICATION=1
    export STSLAM_SEMANTIC_GEOMETRIC_RESCUE_STAGES='*'
    export STSLAM_SEMANTIC_CANDIDATE_GEOMETRY_GATE=1
    export STSLAM_SEMANTIC_CANDIDATE_SPARSE_FLOW_GATE=1
    export STSLAM_SEMANTIC_SPARSE_FLOW_GATE_STAGES=track_local_map_pre_pose
    export STSLAM_SEMANTIC_FLOW_MAX_DYNAMIC_REJECT_RATIO=0.15
    ;;
